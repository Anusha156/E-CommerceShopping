...
public class AdminController {
	...
	public ResponseEntity<List<Object>> loginCustomer(@RequestBody Customer customer) {// using ELK elastic
																						// search,Log,Kibana we can
																						// implement centralized
		log.info("inside admin login of adminservice controller");
	...
	/*
	 * public List<Product> getMessageFallBack() { List<Product> pp=new
	 * ArrayList<Product>(); Product p=new Product(); p.setId(1002);
	 * p.setName("to many attempt within time limit"); p.setActive("not active");
	 * pp.add(p); return pp; }
	 */
	...
	public ResponseEntity<String> getMessageFallBack(RequestNotPermitted exception) {
		...
		// logger.info("Rate limit has applied, So no further calls are getting
		// accepted");
